...
int main()
{
  ...
  hmult->Draw();
  c1->Print("p8_hmult_notso.png");

  hmult_selected->Draw();
  c1->Print("p8_hmult_sel_notso.png");

  // fixed name in accordance with above
  heta->Draw();
  heta->SetMinimum(0); // root zero suppresses by default
  c1->Print("p8_eta_notso.png");

  hPhi->Draw();
  hPhi->SetMinimum(0);
  c1->Print("p8_phi_notso.png");

  heta_vec->Draw();
  heta_vec->SetMinimum(0); // root zero suppresses by default
  c1->Print("p8_eta_vec_ntoso.png");

  hPhi_vec->Draw();
  hPhi_vec->SetMinimum(0);
  c1->Print("p8_phi_vec_notso.png");

  tp1f_c22mult->Draw();
  c1->Print("p8_c22mult_ntoso.png");
  ...
  c1->Print("hmult_recursion_test_notso.png");
  //////////////
