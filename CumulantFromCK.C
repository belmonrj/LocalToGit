...
void CumulantFromCK()
{
  ...
  for(i=1; i<=v; ++i){
    ...
    double innerTerm = pow((val-(9*valTwo*valFour)+12*(valTwo*valTwo*valTwo)), (-5.0/6.0));
    double outerTerm = pow((1.0/6.0), 2) + pow((9*valTwo/6), 2) + pow(((9*valFour)/6 + 24*(valTwo*valTwo)/6),2);
    double newErr = pow((innerTerm*innerTerm)*(outerTerm)*(origErr*origErr), .5);
    double op = -9.0;
    if ( val > 0 ) op = pow((val/4), (1.0/6.0));
  ...
  for(i=1; i<=w; ++i){
    ...
    double outerTerm = pow((1.0/8.0), 2) + pow((16*valTwo), 2) + pow(((36*valFour)-(144*valTwo*valTwo)), 2) + pow((16*valSix), 2);
    double newErr = pow((innerTerm*innerTerm)*(outerTerm)*(origErr*origErr), .5);
    double op = -9.0;
    if ( val < 0 ) op = pow((-val/33), (1.0/8.0));
  ...
  vm_2->GetXaxis()->SetRangeUser(0,100);
...
//   N2->SetParLimits(0,0.0,10.0);
//   N4->SetParLimits(0,0.0,10.0);
//   N6->SetParLimits(0,0.0,10.0);
//   N8->SetParLimits(0,0.0,10.0);
